...
mod tests {
    ...
    fn test_pagination_state_middle_page() {
        ...
        // Initial: left_index = 6 - 3 = 3, right_index = 6 + 3 = 9
        // Dots: left_dots = 3 > 2 = true, right_dots = 9 < 11 = true
        // Adjusted: left_index = 3 + 1 = 4, right_index = 9 - 1 = 8
        assert!(state.left_dots);
        assert!(state.right_dots);
        assert_eq!(state.left_index, 4);
        assert_eq!(state.right_index, 8);
    }
